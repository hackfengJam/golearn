...
func (register *Register) keepOnline() {
	...
	for {
		...
		// 自动续租
		if keepAliveChan, err = register.lease.KeepAlive(context.TODO(), leaseGranResp.ID); err != nil {
			goto RETRY
		}
		...
		// 注册到 etcd
		if _, err = register.kv.Put(cancelCtx, regKey, "", clientv3.WithLease(leaseGranResp.ID)); err != nil {
			goto RETRY
		}
		...
		for {
			...
			case keepAliveResp = <-keepAliveChan:
				if keepAliveChan == nil { // 续租失败（可能由于网络不通等原因导致续租失败）
					goto RETRY
				}
			}
		}
